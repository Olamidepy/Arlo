...
export const CountdownOverlay: React.FC = () => {
  const { status, startGameplay } = useGameStore();
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-md select-none">
      <div className="flex flex-col items-center gap-4 animate-bounce">
        <span className="text-xs uppercase tracking-widest text-[#FF6B00] font-semibold">
          Get Ready
        </span>
        <span className="text-8xl sm:text-9xl font-extrabold font-mono text-white tracking-tighter drop-shadow-[0_0_50px_rgba(255,107,0,0.5)]">
          {count}
        </span>
      </div>
    </div>
